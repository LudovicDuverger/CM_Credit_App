...
const CaseChronology: React.FC<Props> = ({ events }) => {
  ...
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-6 flex flex-col flex-1">
      <h3 className="font-semibold text-slate-900">{t('caseDetail.chronology')}</h3>
      <div className="h-4" aria-hidden="true" />
      {!events.length ? (
        <p className="text-sm text-slate-500">{t('caseDetail.noHistory')}</p>
      ) : (
                      ...
                      <div className="min-w-0 flex flex-wrap items-center gap-1">
                        ...
                        {event.linkUrl ? (
                          <a
                            ...
                            title={t('caseDetail.openBpmProcess')}
                            aria-label={`${t('caseDetail.openBpmProcess')} - ${event.title}`}
                          >
