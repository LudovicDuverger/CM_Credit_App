        ...
        {!visibleAppTasks.length ? (
          ...
          <div className="space-y-3">
            {visibleAppTasks.map((task, index) => {
              ...
              const hasExternalLink = !!task.externalLink && isRunning;
                  ...
                  <div className="min-w-0">
                    ...
                    {hasExternalLink ? (
                      <a
                        ...
                        className={`text-left font-bold hover:text-cyan-700 flex items-center gap-1 ${isCompleted ? 'text-slate-500 line-through' : 'text-cyan-700 underline'}`}
                      >
                        {task.name || '-'}
                      ...
                      <button
                        onClick={openMaestroDetail}
                        className={`text-left font-bold hover:text-cyan-700 ${isCompleted ? 'text-slate-500 line-through' : 'text-slate-900'}`}
                      >
                        {task.name || '-'}
                      </button>
                    )}
